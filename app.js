...
const App = {
    ...
    handleRoute() {
        ...
        } else if (hash.startsWith("#/examples/b2b-sales")) {
    ...
    updateNavActive(hash) {
        ...
        document.querySelectorAll(".nav-menu a").forEach((link) => {
            ...
            const isExamples = href === "#/examples" && route.startsWith("#/examples");
            link.classList.toggle("active", href === route || isExamples);
        });
    },
    ...
    renderProcessView(container) {
        ...
        container.innerHTML = `
            <div class="view-header">
                ...
                <p class="subtitle">AI Agent를 구현하기 위한 7단계 개발 절차입니다.</p>
            </div>
    ...
    renderExamplesPage(container, pageId) {
        const content = AppData.staticPages[pageId];
        const items = [
        ...
        const subMenu = items.map((item) => `
            <a class="examples-subnav-link${item.id === pageId ? " active" : ""}" href="${item.href}">
        ...
        container.innerHTML = `
            <div class="examples-layout">
                <aside class="examples-subnav" aria-label="Examples submenu">
